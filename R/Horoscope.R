...
Horoscope=function(Year,Month,Day){
  ...
  element=(rvest::html_text(rvest::html_node(x=urlcontent,css="#main > div > div > div > article > div:nth-child(7) > div > ul > li:nth-child(5) > a")))
  element=substr(element,5,nchar(element))
  stone=(rvest::html_text(rvest::html_node(x=urlcontent,css="#main > div > div > div > article > div:nth-child(52) > div > p:nth-child(1) > a:nth-child(3)")))
